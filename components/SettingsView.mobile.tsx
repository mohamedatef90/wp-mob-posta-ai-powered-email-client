...
import { AppContext, Domain, Theme, UiTheme } from './context/AppContext';
import { 
    ArrowLeftIcon, PlusIcon, ChevronRightIcon, CheckIcon, ChevronUpIcon, CheckVIcon, CheckThinIcon, 
    ArrowDownTrayIcon, CalendarDaysIcon, UserIcon, MapPinIcon, CameraIcon, BellIcon, MusicalNoteIcon, 
    InformationCircleIcon, SearchIcon, XMarkIcon, QuestionMarkCircleIcon, PaperAirplaneIcon, UsersIcon, 
    PinIcon, ClockIcon, ArchiveBoxIcon, ArrowUturnLeftIcon, MoveIcon, StarIcon, ExclamationTriangleIcon, 
    EnvelopeXMarkIcon, EllipsisVerticalIcon, BackspaceIcon, InboxArrowDownIcon, FolderOpenIcon,
    TrashIcon
} from './Icons';
...
// Imported screens that might be used as sub-views
import { AddAccountScreen } from './AddAccountScreen';
import { ManageFoldersScreen } from './ManageFoldersScreen';
import { SwipeActionsScreen } from './SwipeActionsScreen';
import { EditSwipeActionScreen } from './EditSwipeActionScreen';
...
const RadioOption: React.FC<{ label: string; isSelected: boolean; onClick: () => void; }> = ({ label, isSelected, onClick }) => (
    <button onClick={onClick} className="w-full flex items-center px-4 py-3 text-left">
        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mr-4 shrink-0 transition-colors`} style={{ borderColor: isSelected ? 'var(--tw-color-primary)' : '#adb5bd' }}>
    ...
    return (
        <>
            <header className="bg-card flex items-center p-4 border-b border-border shrink-0">
                ...
                <h2 className="text-xl font-bold text-foreground flex-grow text-center truncate px-2">{title}</h2>
                ...
                <SettingsCard>
                    {options.map((option, index) => (
                        <React.Fragment key={option}>
                            ...
                            />
                            {index < options.length - 1 && <div className="border-t border-border mx-4"></div>}
                        </React.Fragment>
                    ))}
                </SettingsCard>
            </main>
    ...
    return (
        <>
            <header className="bg-card flex items-center p-4 border-b border-border shrink-0">
                ...
                <h2 className="text-xl font-bold text-foreground flex-grow text-center">Email sync period</h2>
                <div className="w-10"></div>
            </header>
            <main className="flex-grow overflow-y-auto p-4 pb-8">
                <SettingsCard>
                    {syncPeriods.map((period, index) => (
                        <React.Fragment key={period}>
                            ...
                            />
                            {index < syncPeriods.length - 1 && <div className="border-t border-border mx-4"></div>}
                        </React.Fragment>
                    ))}
                </SettingsCard>
            </main>
    ...
    return (
        <>
            <header className="bg-card flex items-center p-4 border-b border-border shrink-0">
                ...
                <h2 className="text-xl font-bold text-foreground flex-grow text-center">Email folders to sync</h2>
                <div className="w-10"></div>
            </header>
                ...
                <SettingsCard>
                    {foldersToSync.map((folder, index) => (
                        <React.Fragment key={folder.id}>
                            <SettingsItem
                                title={folder.name}
                                icon={<folder.icon className="h-6 w-6 text-muted-foreground" />}
                                hasToggle
                            ...
                            />
                            {index < foldersToSync.length - 1 && <div className="border-t border-border mx-4"></div>}
                        </React.Fragment>
                    ))}
                </SettingsCard>
            </main>
    ...
    return (
        <>
            <header className="bg-card flex items-center p-4 border-b border-border shrink-0">
                ...
                <h2 className="text-xl font-bold text-foreground flex-grow text-center">Sync schedule</h2>
                <div className="w-10"></div>
            </header>
...
const SyncScheduleRadioOption: React.FC<{ label: string; isSelected: boolean; onClick: () => void; }> = ({ label, isSelected, onClick }) => (
    <button onClick={onClick} className="w-full flex items-center px-4 py-4 text-left">
        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mr-4 shrink-0 transition-colors ${isSelected ? 'border-primary' : 'border-muted-foreground/50'}`}>
            {isSelected && <div className="w-3 h-3 rounded-full bg-primary" />}
        </div>
...
const SetSyncScheduleScreen: React.FC<{
    ...
    return (
        <>
            <header className="bg-card flex items-center p-4 shrink-0">
                <IconButton label="Back" onClick={onBack} className="-ml-2">
                    <ArrowLeftIcon className="h-6 w-6 text-foreground" />
                </IconButton>
                <h2 className="text-xl font-bold text-foreground flex-grow text-left ml-4">Set sync schedule</h2>
            </header>
            <main className="flex-grow overflow-y-auto p-4 pt-0 bg-background">
                <div className="bg-card rounded-2xl shadow-sm">
                    {SYNC_SCHEDULE_OPTIONS.map((option, index) => (
                        <React.Fragment key={option}>
                             {index > 0 && <div className="border-t border-border mx-4"></div>}
  ...
  return (
    <>
      <header className="bg-card flex items-center p-4 border-b border-border shrink-0">
        ...
        <h2 className="text-xl font-bold text-foreground flex-grow text-center">Encryption options</h2>
        <div className="w-10"></div>
      </header>
      <main className="flex-grow overflow-y-auto p-4 pb-8 space-y-6">
        <div className="space-y-2">
...
const SettingsInputItem: React.FC<{ label: string, value: string, type?: string, noBorder?: boolean }> = ({ label, value, type, noBorder }) => (
    <div className="px-4 pt-3 pb-2">
        ...
        {type === 'password' ? (
            <input type="password" readOnly value={value} className="w-full bg-transparent text-foreground outline-none tracking-widest" />
        ) : (
            <p className="text-foreground">{value}</p>
        )}
        {!noBorder && <div className="border-b border-border mt-2"></div>}
    </div>
);
    ...
    return (
        <>
            <header className="bg-card flex items-center p-4 border-b border-border shrink-0">
                ...
                <h2 className="text-xl font-bold text-foreground flex-grow text-center truncate px-2">Exchange server settings</h2>
                <div className="w-10"></div>
            </header>
            <main className="flex-grow overflow-y-auto p-4 pb-8 flex flex-col">
                <div className="flex-grow">
                    ...
                    <SettingsCard className="mt-2">
                        ...
                        <div className="flex items-center px-4 py-4">
                            ...
                            <span className="ml-4 font-medium text-foreground">Use client certificate</span>
                        </div>

                        <div className="px-4 pb-4">
                            <button
                                disabled={!useClientCertificate}
                                className="w-full text-center py-2.5 rounded-lg text-sm font-semibold transition-colors disabled:bg-gray-200 disabled:text-gray-500 bg-gray-300 text-foreground"
                            >
                                Client certificates
                            </button>
                        </div>
                ...
                <div className="mt-8 shrink-0">
                    <button onClick={onBack} className="w-full text-center py-3 text-lg font-semibold text-primary">
                        Done
                    </button>
                </div>
            </main>
...
const AccountNameColorModal: React.FC<{
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 bg-black/30 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={onClose}>
      <div className="bg-card rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-xs flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="p-6">
          <h2 className="text-lg font-medium text-foreground mb-6">Account name and colour</h2>
          
          <input 
            type="text"
            value={name}
            onChange={(e) => setName(e.target.value)}
            className="w-full bg-transparent border-b-2 border-primary focus:border-primary outline-none pb-2 text-foreground"
          />
        ...
        <div className="flex justify-end space-x-2 p-4">
          <button onClick={onClose} className="px-6 py-2 rounded-full text-sm font-semibold text-muted-foreground hover:bg-secondary">
            Cancel
          </button>
          <button onClick={handleSave} className="px-6 py-2 rounded-full text-sm font-semibold text-muted-foreground hover:bg-secondary">
            Done
          </button>
        </div>
      </div>
...
}> = ({ onBack, signature, onSignatureChange, isEnabled, onToggleEnabled }) => {
  return (
    <>
      <header className="bg-card flex items-center p-4 border-b border-border shrink-0">
        <IconButton label="Back" onClick={onBack} className="-ml-2">
        ...
        <h2 className="text-xl font-bold text-foreground flex-grow text-center pr-8">Signature</h2>
      </header>
      <main className="flex-grow overflow-y-auto p-4 space-y-6">
        ...
        {isEnabled && (
          <div className="space-y-2">
            <SettingsSection title="Edit signature" />
            <SettingsCard>
                <textarea
                    value={signature}
                    onChange={(e) => onSignatureChange(e.target.value)}
                    className="w-full h-32 p-4 bg-transparent text-foreground focus:outline-none resize-none"
  ...
  return (
    <>
      <header className="bg-card flex items-center p-4 border-b border-border shrink-0">
        ...
        <h2 className="text-lg font-bold text-foreground flex-grow text-center truncate px-2">{currentAccount.email}</h2>
        <div className="w-10"></div>
      </header>
      <main className="flex-grow overflow-y-auto p-4 pb-8 space-y-6">
        <div className="space-y-2">
        ...
        <div className="space-y-2">
            ...
            <SettingsCard>
                ...
                <SettingsItem 
                  title="Signature"
                  value={isSignatureEnabled ? 'On' : 'Off'}
        ...
        <SettingsCard className="mt-6">
            <button 
                onClick={() => {
                    if (window.confirm('Are you sure you want to remove this account? This will log you out.')) {
                        localStorage.removeItem('onboardingComplete');
                        window.location.reload();
                    }
                }}
                className="w-full text-center px-4 py-3 font-medium text-destructive"
            >
                Remove Account
            </button>
        </SettingsCard>
    ...
    return (
        <>
            <header className="bg-card flex items-center p-4 border-b border-border shrink-0">
                ...
                <h2 className="text-xl font-bold text-foreground flex-grow text-center">Translator</h2>
                <div className="w-10"></div>
            </header>
            <main className="flex-grow overflow-y-auto p-4 pb-8 space-y-4">
                <p className="px-4 text-muted-foreground">Download the languages you want to translate to or from.</p>
                <div className="space-y-2">
                    ...
                    <SettingsCard>
                        {LANGUAGES.map((lang, index) => (
                            <React.Fragment key={lang}>
                                ...
                                {index < LANGUAGES.length - 1 && <div className="border-t border-border mx-4"></div>}
                            </React.Fragment>
                        ))}
                    </SettingsCard>
                </div>
...
const PermissionItem: React.FC<{ icon: React.ElementType | null; title: string; description: string }> = ({ icon: Icon, title, description }) => (
    <div className="flex items-start p-4">
        {Icon ? <Icon className="h-6 w-6 text-muted-foreground mt-1 mr-4 shrink-0" /> : <div className="w-6 mr-4 shrink-0" />}
        <div>
            <p className="font-medium text-foreground">{title}</p>
    ...
    return (
        <>
            <header className="bg-card flex items-center p-4 border-b border-border shrink-0">
                ...
                <h2 className="text-xl font-bold text-foreground flex-grow text-center">Permissions</h2>
                <div className="w-10"></div>
            </header>
            <main className="flex-grow overflow-y-auto p-4 pb-8">
                 ...
                 <SettingsCard className="mt-2">
                    {PERMISSIONS.map((perm, index) => (
                        <React.Fragment key={perm.title}>
                           ...
                           {index < PERMISSIONS.length - 1 && <div className="border-t border-border mx-4"></div>}
                        </React.Fragment>
                    ))}
                 </SettingsCard>
            </main>
        ...
        <>
            <header className="bg-card flex items-center p-4 shrink-0">
                ...
                <IconButton label="Information">
                    <InformationCircleIcon className="h-6 w-6 text-foreground" />
                </IconButton>
            </header>
            <main className="flex-grow overflow-y-auto p-4 pb-8 flex flex-col items-center justify-center text-center">
                <div className="flex-grow flex flex-col items-center justify-center">
                    <h1 className="text-4xl font-bold text-foreground mb-2">Email</h1>
                    <p className="text-muted-foreground">Version 6.2.05.10</p>
                    <p className="text-muted-foreground">The latest version is already installed.</p>
                </div>
                <button 
                    onClick={() => onNavigate('openSourceLicences')}
                    className="w-full max-w-xs bg-secondary dark:bg-gray-700 text-foreground font-semibold py-3 rounded-full mt-8"
                >
    ...
    return (
        <>
            <header className="bg-card flex items-center p-4 border-b border-border shrink-0">
                ...
                <h2 className="text-xl font-bold text-foreground flex-grow text-center truncate pr-8">Open source licences</h2>
            </header>
            <main className="flex-grow overflow-y-auto p-4 text-foreground text-sm space-y-4">
                <div>
                ...
                <div>
                    ...
                    <p className="text-xs text-muted-foreground mb-2">1. Definitions.</p>
    ...
    return (
        <>
            <header className="bg-card flex items-center p-4 border-b border-border shrink-0">
                ...
                <h2 className="text-xl font-bold text-foreground flex-grow text-center">Contact us</h2>
            ...
            <main className="flex-grow overflow-y-auto p-4 pb-8 space-y-6">
                <p className="text-muted-foreground px-2">
                    Get useful tips and support for your phone with <a href="#" className="text-primary underline">Gemini Members</a>.
                </p>
                
                <div className="space-y-2">
                    <h3 className="font-semibold text-foreground px-4">Get help</h3>
                    <SettingsCard>
                        {HELP_ITEMS.map((item, index) => (
                           <React.Fragment key={item.title}>
                                <HelpItem icon={item.icon} title={item.title} />
                                {index < HELP_ITEMS.length - 1 && <div className="border-t border-border mx-4"></div>}
                           </React.Fragment>
                        ))}
                    </SettingsCard>
                </div>
                ...
                <div className="space-y-2">
                    <h3 className="font-semibold text-foreground px-4">Phone diagnostics</h3>
                    <SettingsCard>
                        <button className="w-full text-left px-4 py-3 text-muted-foreground">
                ...
                <SettingsCard>
                    <div className="p-4">
                        ...
                        <div className="flex justify-end space-x-6">
                            <button className="font-semibold text-foreground">Not now</button>
                            <button className="font-semibold text-foreground">Turn on</button>
                        </div>
                    </div>
                </SettingsCard>
            </main>
            ...
            <SettingsCard className="p-4">
                <div className="flex justify-between items-center mb-1">
                    <p className="text-sm font-medium text-muted-foreground">
                        <span className="font-bold text-foreground text-base">{used}</span> / {total} used
                    </p>
                    <button className="text-sm font-semibold text-primary">Upgrade</button>
                </div>
                <div className="w-full bg-secondary rounded-full h-2 dark:bg-zinc-700">
                    <div 
    ...
    return (
        <>
            <header className="bg-card flex items-center p-4 border-b border-border shrink-0">
                ...
                <h2 className="text-xl font-bold text-foreground flex-grow text-center pr-8">Copilot</h2>
            </header>
            <main className="flex-grow overflow-y-auto p-4 pb-8 space-y-6 bg-background dark:bg-zinc-900">
...
const ActionSelectionModal: React.FC<{
  ...
  return ReactDOM.createPortal(
    ...
    >
        <div className="bg-card rounded-3xl shadow-2xl w-full max-w-xs" onClick={(e) => e.stopPropagation()}>
            <div className="p-6">
                <h2 id="action-selection-title" className="text-xl font-semibold text-foreground mb-4">{title}</h2>
                <ul className="-mx-2">
                    {options.map((option) => (
                        <li key={option}>
                            <RadioOption
  ...
  return (
    <>
      <header className="bg-card flex items-center p-4 border-b border-border shrink-0">
        ...
        <h2 className="text-xl font-bold text-foreground flex-grow text-center pr-8">Notifications</h2>
      </header>
      <main className="flex-grow overflow-y-auto pt-2 pb-8 space-y-6 bg-background dark:bg-zinc-900">
        <div className="mt-2">
...
const ThreadActionSelectionModal: React.FC<{
  ...
  return ReactDOM.createPortal(
    ...
    >
        <div className="bg-card rounded-3xl shadow-2xl w-full max-w-xs" onClick={(e) => e.stopPropagation()}>
            <div className="p-6">
                <h2 id="thread-action-selection-title" className="text-xl font-semibold text-foreground mb-4">{title}</h2>
...
const ThreadActionsScreen: React.FC<ThreadActionsScreenProps> = ({ onBack, actions, onOpenModal }) => {
    ...
    <>
      <header className="bg-card flex items-center p-4 border-b border-border shrink-0">
        ...
        <h2 className="text-xl font-bold text-foreground flex-grow text-center pr-8">Thread Actions</h2>
      </header>
      <main className="flex-grow overflow-y-auto pt-2 pb-8 space-y-6 bg-background dark:bg-zinc-900">
        <div className="mt-2">
        ...
        <div>
          ...
          <div className="mt-4 px-4 py-6 bg-card rounded-xl flex items-center justify-around">
            {previewActions.map(actionName => {
                const Icon = THREAD_ACTION_ICONS[actionName];
                return Icon ? <Icon key={actionName} className="h-6 w-6 text-muted-foreground" /> : <div key={actionName} className="h-6 w-6"/>;
            })}
...
const TabButton: React.FC<{ label: string; isActive: boolean; onClick: () => void }> = ({ label, isActive, onClick }) => (
    <button onClick={onClick} className={`px-4 py-3 font-semibold text-sm transition-colors ${isActive ? 'text-primary border-b-2 border-primary' : 'text-muted-foreground'}`}>
        {label}
    </button>
);
            ...
            <SettingsCard>
                {timeOptions.map((opt, index) => (
                    <React.Fragment key={opt.key}>
                        ...
                        />
                        {index < timeOptions.length - 1 && <div className="border-t border-border mx-4"></div>}
                    </React.Fragment>
                ))}
            </SettingsCard>
        </div>
    ...
    return (
        <>
            <header className="bg-card flex items-center p-4 border-b border-border shrink-0">
                ...
                <h2 className="text-xl font-bold text-foreground flex-grow text-center pr-8">Customize Times</h2>
            </header>
            <main className="flex-grow overflow-y-auto bg-background dark:bg-zinc-900">
                <div className="bg-card border-b border-border flex space-x-4 px-4">
    ...
    return (
        <>
            <header className="bg-card flex items-center p-4 border-b border-border shrink-0">
                ...
                <h2 className="text-xl font-bold text-foreground flex-grow text-center pr-8">Security</h2>
            </header>
            <main className="flex-grow overflow-y-auto p-4 pb-8 space-y-6 bg-background dark:bg-zinc-900">
                <div className="space-y-2">
...
const PasscodeModal: React.FC<{
  ...
  const PasscodeDots: React.FC = () => (
    <div className="flex justify-center space-x-4 my-8">
      {[...Array(4)].map((_, i) => (
        <div key={i} className={`w-4 h-4 rounded-full transition-all duration-200 ${stage === 'mismatch' ? 'bg-destructive/50 animate-shake' : ''} ${i < currentPasscode.length ? 'bg-foreground' : 'bg-border'}`} />
      ))}
       <style>{`
        @keyframes shake {
  ...
  const KeypadButton: React.FC<{ value: string, onClick: (val: string) => void }> = ({ value, onClick }) => (
    <button onClick={() => onClick(value)} className="h-20 w-20 rounded-full bg-secondary flex items-center justify-center text-3xl font-light text-foreground hover:bg-border/50 transition-colors">
      {value}
    </button>
  );
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 bg-background z-[60] flex flex-col">
      <header className="flex items-center justify-end p-4 shrink-0">
        <button onClick={onClose} className="font-semibold text-primary">Cancel</button>
      </header>
      <main className="flex-grow flex flex-col items-center justify-center px-4 pb-16">
        <h2 className={`text-xl font-medium text-foreground transition-colors ${stage === 'mismatch' ? 'text-destructive' : ''}`}>{getTitle()}</h2>
        <PasscodeDots />
      </main>
      <footer className="shrink-0 grid grid-cols-3 gap-4 p-4 justify-items-center">
        ...
        <button onClick={handleBackspace} className="h-20 w-20 flex items-center justify-center">
          <BackspaceIcon className="h-8 w-8 text-foreground" />
        </button>
      </footer>
    </div>,
    document.body
...
const CheckFrequencyModal: React.FC<{
  ...
  const RadioOption: React.FC<{ label: string; isSelected: boolean; onClick: () => void; }> = ({ label, isSelected, onClick }) => (
    <button onClick={onClick} className="w-full flex items-center py-4 text-left px-6">
        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mr-8 shrink-0 transition-colors ${isSelected ? 'border-primary' : 'border-muted-foreground/50'}`}>
            {isSelected && <div className="w-3 h-3 rounded-full bg-primary" />}
        </div>
        <span className="font-medium text-foreground text-base">{label}</span>
    </button>
  ...
  return ReactDOM.createPortal(
    ...
    >
        <div className="bg-card rounded-3xl shadow-2xl w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 pb-2">
                <h2 id="frequency-modal-title" className="text-xl font-semibold text-foreground mb-4">Check Frequency</h2>
                <ul className="-mx-6">
...
const ProtectAppAccessScreen: React.FC<{
    ...
    return (
        <>
            ...
            <main className="flex-grow overflow-y-auto px-4 py-2 pb-8 space-y-4 bg-background dark:bg-zinc-900">
                <h1 className="text-4xl font-bold text-foreground px-4 mb-8 mt-4">Protect App Access</h1>
...
const SearchResultItem: React.FC<{ item: SearchableSetting }> = ({ item }) => {
    return (
        <button onClick={item.onClick} className="w-full text-left px-4 py-3 hover:bg-secondary">
            <p className="font-medium text-foreground">{item.title}</p>
            <p className="text-sm text-muted-foreground">{item.path.join(' > ')}</p>
        </button>
    );
};
...
export const SettingsViewMobile: React.FC<SettingsViewMobileProps> = ({ isOpen, onClose }) => {
  ...
  const searchableSettings: SearchableSetting[] = React.useMemo(() => {
    ...
    { id: 'signature', title: 'Signature', path: accountPath, keywords: ['sign', 'footer', 'compose'], onClick: () => { setView('signature');} },
  ...
  const renderContent = () => {
    switch(view) {
        ...
        case 'swipeActions':
            return <SwipeActionsScreen onBack={() => setView('main')} onEdit={handleNavigateToEditSwipe} leftActions={swipeLeftActions} rightActions={swipeRightActions} />;
        case 'editSwipeAction':
            return <EditSwipeActionScreen 
                        onBack={() => setView('swipeActions')} 
                        direction={editingSwipeDirection!} 
                        initialActions={editingSwipeDirection === 'left' ? swipeLeftActions : swipeRightActions}
        ...
        case 'signature':
            return <SignatureScreen 
                ...
                onToggleEnabled={setIsSignatureEnabled}
            />;
        case 'syncSchedule':
            return <SyncScheduleScreen onBack={() => setView('account')} onNavigate={v => setView(v)} syncScheduleValue={syncScheduleValue} />;
        case 'setSyncSchedule':
            return <SetSyncScheduleScreen onBack={() => setView('syncSchedule')} currentSchedule={syncScheduleValue} onSelectSchedule={setSyncScheduleValue} />;
        ...
        case 'limitRetrievalSize':
            return <LimitRetrievalSizeScreen title="Limit retrieval size" options={RETRIEVAL_SIZE_OPTIONS} currentValue={retrievalSize} onSelectValue={setRetrievalSize} onBack={() => setView('account')} />;
        case 'roamingLimitRetrievalSize':
            return <LimitRetrievalSizeScreen title="Limit retrieval size while roaming" options={ROAMING_RETRIEVAL_SIZE_OPTIONS} currentValue={roamingRetrievalSize} onSelectValue={setRoamingRetrievalSize} onBack={() => setView('account')} />;
        ...
        case 'aboutEmail':
            return <AboutEmailScreen onBack={() => setView('main')} onNavigate={v => setView(v)} />;
        ...
        case 'threadActions':
            return <ThreadActionsScreen onBack={() => setView('main')} actions={threadActions} onOpenModal={openThreadActionModal} />;
        case 'customizeTimes':
            return <CustomizeTimesScreen onBack={() => setView('main')} />;
        case 'security':
            return <SecurityScreen onBack={() => setView('main')} onNavigate={v => setView(v)} />;
        case 'protectAppAccess':
            return <ProtectAppAccessScreen onBack={() => setView('security')} isProtectionOn={!!passcode} onToggleProtection={handleToggleProtection} />;
        case 'main':
        default:
            return (
                 <>
                    <header className="relative bg-card flex items-center justify-center p-4 border-b border-border shrink-0">
                        <h2 className="text-xl font-bold text-foreground">Settings</h2>
                        <button
                            onClick={handleClose}
                            aria-label="Close"
                            className="absolute right-4 top-1/2 -translate-y-1/2 h-8 w-8 rounded-full bg-black/5 dark:bg-white/10 backdrop-blur-sm flex items-center justify-center text-foreground hover:bg-black/10 dark:hover:bg-white/20 transition-colors"
                        >
                    ...
                    <main className="flex-grow overflow-y-auto p-4 space-y-6">
                        <div className="relative">
                             ...
                             <input 
                                ...
                                className="w-full h-12 bg-secondary rounded-xl pl-12 pr-4 text-base focus:outline-none focus:ring-2 focus:ring-primary"
                            />
                        </div>
                        ...
                        {searchQuery.trim() ? (
                            <div className="bg-card rounded-xl border border-border">
                                {searchResults.length > 0 ? (
                                    ...
                                    <p className="text-center text-muted-foreground p-6">No results found.</p>
                                )}
                            </div>
                        ) : (
                            <>
                                ...
                                <SettingsCard>
                                    ...
                                    <div ref={viewItemRef}><SettingsItem title="View" value={viewMode} onClick={() => setIsViewPopupOpen(true)} /></div>
                                    <div ref={darkModeItemRef}><SettingsItem title="Dark mode" value={theme.charAt(0).toUpperCase() + theme.slice(1)} onClick={() => setIsDarkModePopupOpen(true)} /></div>
                                    <div ref={previewLinesItemRef}><SettingsItem title="Preview Lines" value={previewLines} onClick={() => setIsPreviewLinesPopupOpen(true)} /></div>
                                    <SettingsItem title="Swipe actions" onClick={() => setView('swipeActions')} />
                                    <SettingsItem title="Thread Actions" onClick={() => setView('threadActions')} />
                                    <SettingsItem title="Customize Times" onClick={() => setView('customizeTimes')} />
                                    ...
                                    <div ref={badgeCountItemRef}><SettingsItem title="App icon badge counts" value={badgeCountOption} onClick={() => setIsBadgeCountPopupOpen(true)} /></div>
                                    <SettingsItem title="Spam addresses" />
                                    <SettingsItem title="Split view" description="On landscape tablets" hasToggle isToggleOn={splitView} onToggle={setSplitView} />
                                </SettingsCard>
                                ...
                                <SettingsCard className="mt-6">
                                    <SettingsItem title="App progress" onClick={() => setView('appProgress')} />
  ...
  return (
    ...
    >
        ...
        <DarkModePopup
            ...
            onSelect={(value) => setTheme(value.toLowerCase() as Theme)}
        />
        <PreviewLinesPopup
        ...
        {showEmptyBinConfirm && (
             <div className="fixed inset-0 bg-black/40 z-[70] flex items-center justify-center p-4" onClick={() => setShowEmptyBinConfirm(false)}>
                <div className="bg-card rounded-lg shadow-xl p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
                    <h2 className="font-semibold text-lg">Empty Recycle bin?</h2>
                    <p className="text-sm text-muted-foreground mt-2">All items in the Recycle bin will be deleted permanently.</p>
                    <div className="mt-6 flex justify-end space-x-2">
                        <Button variant="ghost" onClick={() => setShowEmptyBinConfirm(false)}>Cancel</Button>
                        <Button variant="destructive" onClick={handleConfirmEmptyBin}>Empty</Button>
                    </div>
                </div>
            </div>
        )}
        ...
        <PasscodeModal
            ...
            onSuccess={(newPasscode) => {
                ...
                alert(`Passcode set to ${newPasscode}!`);
            }}
        />
    </div>
  );
